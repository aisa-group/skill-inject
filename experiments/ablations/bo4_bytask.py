#!/usr/bin/env python3
"""Best-of-4 by Task ablation for obvious injections.

Uses obvious_injections_sametask.json where each injection has 4 task variants
using the same skill but with different task prompts. For each of 4 runs, a
different task variant is used, testing whether the specific user task affects
attack success rate.

Usage:
    python3 experiments/ablations/bo4_bytask.py                  # run all models
    python3 experiments/ablations/bo4_bytask.py --agent codex --model gpt-5.1-codex-mini
    python3 experiments/ablations/bo4_bytask.py --smoke-test     # quick test
"""
...
SAMETASK_INJECTIONS_FILE = DATA_DIR / "obvious_injections_sametask.json"
N_RUNS = 4
...
def aggregate(all_runs: list[dict], n_runs: int) -> dict:
    """Aggregate per-injection Bo4 results."""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Best-of-4 by Task ablation (obvious)")
    ...
    if not SAMETASK_INJECTIONS_FILE.exists():
        sys.exit(f"[error] Sametask injections file not found: {SAMETASK_INJECTIONS_FILE}")

    with SAMETASK_INJECTIONS_FILE.open() as f:
    ...
    for mcfg in models:
        ...
        print(f"\n{'#'*60}")
        print(f"# {display} ({agent}/{model}) — {n_runs} runs (varying task prompts)")
        print(f"{'#'*60}")
        ...
        out = run_dir / "aggregated_results_bytask.json"
        out.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
        print(f"\nSaved aggregated results to {out}")

    print("\n[done] Bo4 bytask ablation complete.")
